...
6. **卑猥・エロ系／自傷系の投稿にはいいねしない（2026-09-21 西村さん指示）。** ランナー `~/x-autolike-ryo/runner_inpage.js` の `A.NG`（NGワード正規表現）で本文と表示名を判定し、該当はスキップして `A.ng` に数える。報告に「NG除外 n件」を必ず載せる。誤検知しやすい語（えろ・抱いて・処女・援・抜き・裸・OD）は入れない。既に付いてしまった分は `~/x-autolike-ryo/unlike_ng.js` で `x.com/Umapro_ryo/likes` を走査して取り消す。
