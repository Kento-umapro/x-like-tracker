...
6. **卑猥・エロ系／自傷系の投稿にはいいねしない（2026-09-21 西村さん指示）。** ランナー `~/x-autolike-ryo/runner_inpage.js` の `A.NG`（NGワード正規表現）で**本文だけ**を判定し、該当はスキップして `A.ng` に数える。報告に「NG除外 n件」を必ず載せる。**表示名（アカウント名）は判定に使わない**（2026-09-23 修正。表示名を見ていたせいで無関係な投稿まで巻き込んで取り消してしまった）。誤検知しやすい語（えろ・抱いて・処女・援・抜き・裸・OD・不倫・スケベ・全裸・下着・変態・ハメ・ヤった・バイブ・AV単独）は入れない。既に付いてしまった分は `~/x-autolike-ryo/unlike_ng.js` で `x.com/Umapro_ryo/likes` を走査して取り消す（**取り消す前に対象リストを必ずファイルに保存する**。2026-09-21 は44件を取り消したが記録が残らず復元に手間取った）。
