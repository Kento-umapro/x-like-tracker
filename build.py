...
/* ── run log ────────────────────────────── */
.runlog{display:flex; flex-direction:column; gap:10px}
.runday{
  background:var(--panel); border-radius:14px; box-shadow:var(--glow);
  padding:14px 18px; display:flex; flex-wrap:wrap; align-items:center; gap:8px 10px;
}
.runday .rd{
  font-family:"Martian Mono",monospace; font-size:12px; font-weight:700; color:var(--ink);
  min-width:72px;
}
.runday .rd small{display:block; font-size:9px; font-weight:500; color:var(--muted); letter-spacing:.1em}
.runday .runchip{
  display:inline-flex; align-items:center; gap:6px;
  background:var(--sunk); border:1px solid var(--rule); border-radius:7px; padding:4px 9px;
  font-size:12px; white-space:nowrap;
}
.runday .runchip b{font-family:"Martian Mono",monospace; font-weight:600; color:var(--ink)}
.runday .runchip i{font-style:normal; font-weight:700; font-variant-numeric:tabular-nums; font-family:"Martian Mono",monospace; font-size:12px}
.runday .runchip i.fy{color:var(--vio)}
.runday .runchip i.fl{color:var(--amb)}
.runday .runchip i.sq{color:var(--pink)}
.runday .runchip.warn{border-color:var(--neg); box-shadow:inset 0 0 0 1px var(--neg)}
...
<div class="wrap">
  ...
  <section>
    <div class="sec-head"><h2>実行ログ</h2><span class="lab">いつ・どこで・何件いいねしたか</span></div>
...
/* ── run log ── */
$("runlog").innerHTML = days.slice().reverse().map(d => {
  const chips = (d.runs || []).map(r =>
    '<span class="runchip' + (r.w ? ' warn' : '') + '"><b>' + r.t + '</b>' +
    (r.fy ? '<i class="fy">' + r.fy + '</i>' : '') +
    (r.fl ? '<i class="fl">' + r.fl + '</i>' : '') +
    (r.s ? '<i class="sq">' + r.s + (r.q ? ':' + r.q : '') + '</i>' : '') + '</span>'
  ).join('');
  return '<div class="runday"><span class="rd">' + mmdd(d.date) + '<small>' + dow(d.date) + '曜・' +
    ((d.runs||[]).length) + 'ラン</small></span>' + (chips || '<span class="lab">記録なし</span>') + '</div>';
}).join('');
